...
// Signature scheme: HMAC_SHA256(order_id + "|" + payment_id, KEY_SECRET) === razorpay_signature
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { razorpay_order_id, razorpay_payment_id, razorpay_signature, invoice_id } =
    ...
    // 2) load the invoice (must belong to this user + match the order)
    ...
    // 3) activate (atomic: marks invoice paid + upserts subscription + period end)
    ...
    const { data: plan } = await admin
      .from("plans").select("slug, name").eq("id", invoice.plan_id).maybeSingle();

    return json({ ok: true, plan_slug: plan?.slug ?? null, plan_name: plan?.name ?? null });
  } catch (err) {
